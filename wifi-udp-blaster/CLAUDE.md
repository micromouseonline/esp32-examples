...
- `wifi-scanner.ino` -- Arduino entry point; calls `app_setup()` / `app_loop()`
- `src/application.cpp` -- WiFi scan logic: scans every 2 s, prints SSID/RSSI/channel/encryption to Serial
- `src/application.h` -- declares `app_setup()` / `app_loop()`
- `src/board_config.h` -- multi-board LED pin mapping (`STATUS_LED`); supports S3-Zero, C3-SuperMini, C3-Xiao, C6-SuperMini, with silicon autodetect fallback
